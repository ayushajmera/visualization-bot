...
def generate_report(df, be):
    ...
    if st.button("🚀 Run Full Automated Analysis"):
        with st.spinner("Performing Data Quality Audit..."):
            # 1. Perform Data Quality Audit first
            da.perform_audit(df, be)
        st.markdown("---") # Add a separator

        with st.spinner("Generating Automated Quick Insights..."):
            # 2. Generate text-based insights
            ai.generate_text_insights(df)
        st.markdown("---") # Add a separator

        with st.spinner("Generating Cleaning & Preparation Suggestions..."):
        ...
        with st.spinner("Performing Time-Based Analysis..."):
